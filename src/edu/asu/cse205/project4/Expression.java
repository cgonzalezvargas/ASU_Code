...
public class Expression {
     ...
     * End If
     */
    private void topEval(Stack<Operator> operatorStack, Stack<Operand> operandStack) {
        Operand right = operandStack.pop();
        Operator operator = operatorStack.pop();

        if (operator instanceof UnaryOperator) {
            operandStack.push(((UnaryOperator) operator).evaluate(right));
        } else {
            Operand left = operandStack.pop();
            operandStack.push(((BinaryOperator) operator).evaluate(left, right));
        }
    }
